...
<head>
            ...
            <table id="main-tb" class="table table-striped"
                ...
                <thead>
                    <tr class="border-top-1" style="border-top-style: solid; border-width: thin; border-color: #000">
                        <td rowspan="1" colspan="12" class="border-top-0 border-left-1 border-bottom-0"
                            style="border-right-style: solid; border-width: thin;">
                        </td>
                        <td rowspan="1" colspan="2"
                            class="align-top border-top-0 border-left-2 border-right-1 border-bottom-2"
                    ...
                    <tr class="border-0">
                        <td rowspan="1" colspan="14" class="border-top-0">
                    ...
                    <tr>
                        <td colspan="5" class="text-start"><strong>DESCRIPTION<br>(KETERANGAN)</strong></td>
                        <td colspan="1" class="text-start cell-fit tikdu_eq">:</td>
                        <td colspan="8" class="text-start">{{ $worksheet->project->id_project }}</td>
                    </tr>
                    <tr>
                        <td colspan="5" class="text-start"><strong>CLIENT'S NAME<br>(NAMA CUSTOMER)</strong></td>
                        <td colspan="1" class="text-start cell-fit tikdu_eq">:</td>
                        <td colspan="3" class="text-start">{{ $worksheet->project->client->na_client }}</td>
                        <td colspan="1" class="text-start cell-fit text-trans tikdu_eq">:</td>
                        <td colspan="1"><strong>ARRIVAL TIME<br>(WAKTU DATANG)</strong></td>
                        <td colspan="1" class="text-start cell-fit tikdu_eq">:</td>
                    ...
                    @endphp
                    <tr>
                        <td colspan="5" class="text-start"><strong>DATE<br>(TANGGAL)</strong></td>
                        <td colspan="1" class="text-start cell-fit tikdu_eq">:</td>
                        <td colspan="3" class="text-start">{{ $formattedDate }}</td>
                        <td colspan="1" class="text-start cell-fit text-trans tikdu_eq">:</td>
                        <td colspan="1" class="text-start"><strong>FINISH TIME<br>(WAKTU SELESAI)</strong></td>
                        <td colspan="1" class="text-start cell-fit tikdu_eq">:</td>
                        <td colspan="4" class="text-start">
                            {{ \Carbon\Carbon::parse($worksheet->finish_time_ws)->format('h:i:s A') }}</td>
                    </tr>
                    <tr>
                ...
                <tbody>
                    @foreach ($worksheet->task as $task)
                        <tr>
                            ...
                            <td colspan="7">{{ $task->monitor->category }}</td>
                            <td colspan="1">
                                @php
                ...
                <tfoot style="border-top: 1px !important;">
                    ...
                    <tr>
                        <td colspan="14" class="text-left align-middle" style="min-height: 80px !important;"></td>
                    </tr>
                    <tr>
    ...
    <script>
        window.addEventListener('load', function() {
            console.log('Pagination function started');
            ...
            currentTable.innerHTML = document.querySelector('#main-tb thead').innerHTML + '<tbody></tbody>' +
                document.querySelector('#main-tb tfoot').innerHTML;
            let currentTableBody = currentTable.querySelector('tbody');

            let currentHeight = 0;

            rows.forEach(row => {
                const rowHeight = row.getBoundingClientRect().height; // Measure the row height
                console.log('Row Height:', rowHeight);
                console.log('Current Height Before Adding Row:', currentHeight);

                // Check if adding this row would exceed the available height
                console.log('Checking if new page is needed: currentHeight:', currentHeight,
                    ' + rowHeight:', rowHeight, ' > availableHeight:', availableHeight);
                if (currentHeight + rowHeight > availableHeight) {
                    console.log('Creating new page because currentHeight:', currentHeight, ' + rowHeight:',
                        rowHeight, ' > availableHeight:', availableHeight);

                    // Append the current table to the current page
                    currentPage.appendChild(currentTable);
                    console.log('Current Page Height:', currentHeight + headerHeight + footerHeight);

                    // Create a new page and reset the current height
                    ...
                    currentTable.classList.add('table', 'table-striped');
                    currentTable.innerHTML = document.querySelector('#main-tb thead').innerHTML +
                        '<tbody></tbody>' + document.querySelector('#main-tb tfoot').innerHTML;
                ...
                function cloneIT() {
                    // Clone the row to the new table (commented out)
                    // const clonedRow = row.cloneNode(true);
                    // currentTableBody.appendChild(clonedRow);
                    // currentHeight += rowHeight; // Update the current height
                }

                console.log('New Current Height After Adding Row:', currentHeight);
            });

            // Final height calculation
            const finalPageHeight = currentHeight + headerHeight + footerHeight;
            console.log('Final Page Height Calculation:', finalPageHeight);

            if (currentHeight > 0) {
                currentPage.appendChild(currentTable);
                console.log('Final Page Height:', finalPageHeight);
            }
        });
    </script>
